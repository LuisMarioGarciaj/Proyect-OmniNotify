...
export class EmailProvider {
      ...
      if (smtpHost && smtpUser && smtpPass) {
        ...
        this.transporter.verify((error, success) => {
          if (error) {
            ...
            this.logger.warn('⚠️ Modo simulado activado');
          } else {
            this.logger.log('✅ Transporter de email inicializado correctamente');
          }
        });
      } else {
        this.logger.warn('⚠️ No hay configuración SMTP completa, usando modo simulado');
      }
    } catch (error) {
  ...
  async sendEmail(dto: SendNotificationDto): Promise<any> {
    try {
      if (!this.transporter) {
        this.logger.log(`📧 Simulando envío de email a: ${dto.recipient}`);
        return { 
          success: true, 
          simulated: true,
          message: 'Email simulado (no hay configuración SMTP válida)'
        };
      }
      ...
      const mailOptions = {
        ...
        subject: dto.subject,
        html: dto.html,
        text: dto.text || this.htmlToText(dto.html),
        headers: {
