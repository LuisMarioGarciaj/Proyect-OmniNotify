...
interface LoginResponse {
  ...
  user?: {
    ...
    role: string;
    company_id: string;
    company_name?: string; // <-- ¡IMPORTANTE!
  };
  // Por si la respuesta es plana (sin user)
  id?: string;
  email?: string;
  name?: string;
  role?: string;
...
const LoginForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Primer login → mostrar pantalla OTP
      if (data.requires_otp) {
        setOtpData({ userId: data.user_id, email: data.email });
        setShowOtp(true);
        setIsLoading(false);
        return;
      }
      ...
      // 2. Guardar Usuario - AHORA CON company_name INCLUIDO
      let userData: any = {};

      if (data.user) {
        // Si la respuesta tiene user object
        userData = {
          id: data.user.id,
