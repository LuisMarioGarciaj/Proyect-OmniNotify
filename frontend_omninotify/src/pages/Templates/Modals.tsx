// src/pages/Templates/Modals.tsx
...
export const EditorModal: React.FC<EditorModalProps> = ({
  ...
  // 🔥 Función para envolver el contenido en estructura HTML completa
  const wrapInHtmlStructure = (content: string, templateName: string, channel: string): string => {
    if (channel !== 'EMAIL') return content; // Solo EMAIL necesita estructura HTML completa
    
    // Si ya tiene estructura HTML, no la envolvemos de nuevo
...
<head>
  ...
  <style>
    ...
    .template-variable { display: inline-block; padding: 2px 8px; margin: 0 2px; background: #dbeafe; color: #1e40af; border: 1px solid #93c5fd; border-radius: 4px; font-size: 14px; font-weight: 500; cursor: default; user-select: none; }
  </style>
</head>
<body>
  <div class="container">
  ...
  useEffect(() => {
    if (initialTemplate) {
      ...
      // 🔥 Extraer solo el contenido del body si es HTML completo
  ...
  const handleChannelChange = (channel: 'EMAIL' | 'SMS' | 'WHATSAPP') => {
    ...
    if (editorRef.current) {
      const placeholder = channel === 'EMAIL' 
        ? '<p>Empieza a escribir o elige una plantilla...</p>'
        : channel === 'WHATSAPP'
        ? '<p>Escribe tu mensaje de WhatsApp... Puedes usar *texto* para negrita y _texto_ para cursiva.</p>'
        : '<p>Escribe tu mensaje SMS...</p>';
  ...
  const handleSubmit = async () => {
    ...
    // 🔥 Preparar el contenido final según el canal
    let finalContent = currentContent;
    
    if (currentChannel === 'EMAIL') {
      finalContent = wrapInHtmlStructure(currentContent, currentName, currentChannel);
    } else if (currentChannel === 'WHATSAPP') {
      // Para WhatsApp, mantener el formato pero limpiar HTML si es necesario
      finalContent = currentContent.replace(/<[^>]*>/g, ''); // Eliminar HTML para WhatsApp
    } else if (currentChannel === 'SMS') {
      // Para SMS, solo texto plano
      finalContent = currentContent.replace(/<[^>]*>/g, '');
    }
